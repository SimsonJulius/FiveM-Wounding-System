...
function ProcessRunStuff(ped)
    if IsInjuryCausingLimp() and not (onPainKiller > 0)  then
        ...
        if wasOnPainKillers then
            ...
            ESX.ShowNotification('Sie haben erkannt, dass der Drogenkonsum nicht alle Ihre Probleme lost', 5000, 'error')
        end
    else
...
function CheckDamage(ped, bone, weapon)
    ...
    if parts[bone] ~= nil then
        if not BodyParts[parts[bone]].isDamaged then
            ...
            exports['mythic_notify']:DoHudText('inform', 'Deine ' .. BodyParts[parts[bone]].label .. ' fuhlt sich ' .. WoundStates[BodyParts[parts[bone]].severity])
            ...
            TriggerServerEvent('wounding:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('wounding:client:SyncBleed')
AddEventHandler('wounding:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
end)

RegisterNetEvent('wounding:client:FieldTreatLimbs')
AddEventHandler('wounding:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('wounding:client:ResetLimbs')
AddEventHandler('wounding:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('wounding:client:FieldTreatBleed')
AddEventHandler('wounding:client:FieldTreatBleed', function()
    if isBleeding > 1 then
        isBleeding = tonumber(isBleeding) - 1
    end
end)

RegisterNetEvent('wounding:client:ReduceBleed')
AddEventHandler('wounding:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
    end
end)

RegisterNetEvent('wounding:client:RemoveBleed')
AddEventHandler('wounding:client:RemoveBleed', function()
    isBleeding = 0
end)

RegisterNetEvent('wounding:client:UsePainKiller')
AddEventHandler('wounding:client:UsePainKiller', function(tier)
    if tier < 4 then
        onPainKiller = 90 * tier
    end

    ESX.ShowNotification('Du spurst, wie der Schmerz erstmal nachlasst', 4000, 'info')
end)

RegisterNetEvent('wounding:client:UseAdrenaline')
AddEventHandler('wounding:client:UseAdrenaline', function(tier)
    if tier < 4 then
        onDrugs = 180 * tier
    end

    ESX.ShowNotification('Du kannst das Versagen deines Korpers ignorieren', 4000, 'info')
end)  
		...
		if not IsEntityDead(player) and not (#injured == 0) then
			if #injured > 0 then
				...
				if #injured > 1 and #injured < 3 then
					for k, v in pairs(injured) do
						str = str .. 'Dein ' .. v.label .. ' fuhlt sich ' .. WoundStates[v.severity]
						if k < #injured then
							str = str .. ' | '
						end
					end
				elseif #injured > 2 then
					str = 'Du fulhst mehrere Schmerzen'
				else
					str = 'Dein ' .. injured[1].label .. ' fuhlt sich ' .. WoundStates[injured[1].severity]
				end
			...
			if isBleeding > 0 then
				if blackoutTimer >= 10 then
					ESX.ShowNotification('Du bist Ohnmachtig geworden', 3000, 'info')
				...
				exports['mythic_notify']:DoCustomHudText('inform', 'Du hast ' .. BleedingStates[isBleeding], 25000)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        local armourDamaged = (playerArmour ~= armour and armour < playerArmour and armour > 0)
        local healthDamaged = (playerHealth ~= health and health < playerHealth)
